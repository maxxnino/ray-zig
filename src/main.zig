...
pub fn main() anyerror!void {
    ...
    while (!ray.WindowShouldClose()) {
        ...
        if (ray.IsKeyPressed(ray.KEY_C)) {
            const totalItem = std.meta.fields(CameraMode).len;
            const currentItem = @intCast(usize, @enumToInt(cam_mode)) + 1;
            const nextItem = @intCast(std.meta.Tag(CameraMode), currentItem % totalItem);
            cam_mode = @intToEnum(CameraMode, nextItem);
        }
